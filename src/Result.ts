...
// model
// -----------------------------------------------------------------------------
export type Err<ErrType, OkType> =
  ResultOperations<ErrType, OkType> &
...
export type Result<ErrType, OkType> = Err<ErrType, OkType> | Ok<ErrType, OkType>;
...
type ResultOperations<ErrType, OkType> = {
  ...
  andThen: <ErrReturn, OkReturn>(fn: (value: OkType) => Result<ErrReturn, OkReturn>) => Result<ErrType | ErrReturn, OkReturn>;
}
...
// constructors
// -----------------------------------------------------------------------------
export const err = <ErrType = never, OkType = never>(e: ErrType): Result<ErrType, OkType> => ({
  _tag: 'err',
  err: e,
  map: () => err(e),
...
export const ok = <ErrType = never, OkType = never>(value: OkType): Result<ErrType, OkType> => ({
  _tag: 'ok',
  value,
